...
<script type="text/javascript">
    ...
    new GridBuilder( _grid , {
            ...
            { 
                ...
                renderer:function(row) {
                    if (row.post_status == 'draft') {
                        return Ahref(_BASE_URL + 'event/create/' + row.id, 'Edit');    
                    }
                    return '';
                },
                exclude_excel: true,
        ...
        can_restore: false,
        resize_column: 3,
        extra_buttons: '<a class="btn btn-success btn-sm add" href="' + _BASE_URL + 'event/create'+'" data-toggle="tooltip" data-placement="top" title="Add"><i class="fa fa-plus"></i></a>' 
    });

    function find_id( id ) {
        $.post(_BASE_URL + 'event/find_id', {id:id}, function(response) {
            $('.modal-form').modal({
                show:true
            });
