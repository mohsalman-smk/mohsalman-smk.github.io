...
function getFromSourceList() {
	...
	$.post(_BASE_URL + 'academic/class_group_students/get_students', values, function( response ) {
		...
		if (res.length) {
			...
			for (var z in res) {
				str += '<tr>';
				str += '<td><input type="checkbox" name="checkbox-origin-class" value="' + res[ z ].id + '" /></td>';
				str += '<td>' + no + '.</td>';
				str += '<td>' + res[ z ].identity_number + '</td>';
...
function saveToDestinationClass() {
	...
	if (from_class_group_id == to_class_group_id && from_academic_year_id == to_academic_year_id) {
		H.growl('warning', 'Kelas dan ' + _ACADEMIC_YEAR + ' tujuan tidak boleh sama dengan Kelas dan ' + _ACADEMIC_YEAR + ' asal.');
	} else {
...
function getFromTargetList() {
	...
	$.post(_BASE_URL + 'academic/class_group_students/get_students', values, function( response ) {
		...
		if (res.length) {
			...
			for (var z in res) {
				str += '<tr>';
				str += '<td><input type="checkbox" name="checkbox-destination-class" value="' + res[ z ].id + '" /></td>';
				str += '<td>' + no + '</td>';
				str += '<td>' + res[ z ].identity_number + '</td>';
				str += '<td>' + res[ z ].full_name + '</td>';
				str += '</tr>';
...
<section class="content">
	<div class="callout callout-info">
		...
		<ul>
			...
			<li>Jika yang dipilih <strong>"NAMA KELASNYA"</strong>, maka akan diminta untuk memilih <strong>"<?=strtoupper($this->session->userdata('_academic_year'))?>"</strong></li>
			<li>Setelah data peserta didik tampil, silahkan <strong>"CEKLIS"</strong> peserta didik yang akan diatur kelasnya</li>
			<li>Setelah selesai memilih peserta didik yang akan dipindah kelasnya, silahkan pilih <strong>"KELAS TUJUAN"</strong> dan <strong>"<?=strtoupper($this->session->userdata('_academic_year'))?>"</strong> pada tabel sebelah kanan.</li>
			<li>Terakhir klik tombol <strong>"PINDAH KE KELAS TUJUAN"</strong></li>
			<li>Jika terjadi kelasahan penginputan data, silahkan pilih <strong>"KELAS TUJUAN"</strong> dan <strong>"<?=strtoupper($this->session->userdata('_academic_year'))?>"</strong> pada tabel sebelah kanan, kemudian ceklis peserta didik yang akan dihapus dari kelas tersebut dan terakhir klik tombol <strong>"HAPUS"</strong></li>
		</ul>
	</div>
	<div class="row">
				...
				<div class="box-body">
					<form class="form-horizontal">
						<div class="form-group">
							...
							<div class="col-sm-8">
								<?=form_dropdown('class_group_id', ['unset' => 'Belum Diatur', 'show_all' => 'Tampilkan Semua'] + $ds_class_groups, '', 'class="form-control select2" id="from_class_group_id" onchange="showAcademicYear()"');?>
							</div>
						</div>
						<div class="form-group from_academic_year_id" style="display: none;">
							<label for="from_academic_year_id" class="col-sm-4 control-label"><?=$this->session->userdata('_academic_year')?></label>
							<div class="col-sm-8">
								<?=form_dropdown('academic_year_id', $ds_academic_years, $this->session->userdata('current_academic_year_id'), 'class="form-control select2" id="from_academic_year_id" onchange="getFromSourceList()"');?>
							</div>
						</div>
					</form>
					...
					<form class="form-horizontal">
						<div class="box-body">
							<div class="form-group">
								...
								<div class="col-sm-8">
									<?=form_dropdown('class_group_id', $ds_class_groups, '', 'class="form-control select2" id="to_class_group_id" onchange="getFromTargetList()"');?>
								</div>
							</div>
							<div class="form-group">
								<label for="from_academic_year_id" class="col-sm-4 control-label"><?=$this->session->userdata('_academic_year')?></label>
								<div class="col-sm-8">
									<?=form_dropdown('academic_year_id', $ds_academic_years, $this->session->userdata('current_academic_year_id'), 'class="form-control select2" id="to_academic_year_id" onchange="getFromTargetList()"');?>
								</div>
							</div>
						</div>
					</form>
