...
DS.AcademicYears = <?=$academic_year_dropdown;?>;
DS.ClassGroups = <?=$class_group_dropdown;?>;
...
new FormBuilder( _form , {
   controller:'academic/class_group_settings',
   fields: [
      { label:'Tahun ' + (_SCHOOL_LEVEL >= 5 ? 'Dosen Wali' : 'Wali Kelas'), name:'academic_year_id', type:'select', datasource:DS.AcademicYears },
      { label:'Kelas', name:'class_group_id', type:'select', datasource:DS.ClassGroups },
      { label:(_SCHOOL_LEVEL == 5 ? 'Dosen Wali' : 'Wali Kelas'), name:'class_vice_id', type:'select', datasource:DS.Employees }
   ]
});
</script>
