	...
	*/
	function getSubjects() {
		...
		$.post(_BASE_URL + 'academic/course_classes/get_subjects', values, function( response ) {
			...
			if (res.length) {
				...
				for (var z in res) {
					str += '<tr>';
					str += '<td><input type="checkbox" name="checkbox-source-data" value="' + res[ z ].id + '" /></td>';
					str += '<td>' + no + '.</td>';
	...
	function Save() {
		...
		if (copy_data == 'true') {
			if (from_class_group_id == to_class_group_id && from_semester == to_semester && from_academic_year_id == to_academic_year_id) {
				H.growl('warning', _ACADEMIC_YEAR + ', semester dan kelas asal tidak boleh sama dengan ' + _ACADEMIC_YEAR + ', semester dan kelas tujuan.');
				return;
			}
		}
	...
	function changeDeletedStatus(is_deleted) {
		...
		if (subject_ids.length) {
			...
			var semester = $('#to_semester option:selected').text();
			var class_name = $('#to_class_group_id option:selected').text();
			var message = 'Data ' + _SUBJECT + ' ini berelasi dengan data kehadiran ' + (_SCHOOL_LEVEL >= 5 ? 'dosen' : 'guru') + ' dan ' + _STUDENT.toUpperCase() + ', jika data ini dihapus maka <b><font color="red">DATA KEHADIRAN ' + (_SCHOOL_LEVEL >= 5 ? 'DOSEN' : 'GURU') + ' DAN ' + _STUDENT.toUpperCase() + ' AKAN IKUT TERHAPUS</font></b>. Apakah anda yakin ' + subject_ids.length + ' ' + _SUBJECT + ' pada ' + _ACADEMIC_YEAR + ' <b>' + academic_year +'</b> semester <b>' + semester + '</b> kelas <b>' + class_name + '</b> yang terceklis akan dihapus ?';
			var message_type = 'Peringatan !';
			if (is_deleted == 'false') {
				message = 'Apakah anda yakin ' + subject_ids.length + ' ' + _SUBJECT + ' pada ' + _ACADEMIC_YEAR + ' <b>' + academic_year +'</b> semester <b>' + semester + '</b> kelas <b>' + class_name + '</b> yang terceklis akan direstore ?';
	...
	function getCourseClasses() {
		...
		$.post(_BASE_URL + 'academic/course_classes/get_course_classes', values, function( response ) {
			...
			if (res.length) {
				...
				var no = 1;
				for (var z in res) {
					str += '<tr ' + (res[ z ].is_deleted == 'true' ? 'class="delete"' : '') + '>';
					str += '<td><input type="checkbox" name="checkbox-target-data" value="' + res[ z ].id + '" /></td>';
					str += '<td>' + no + '</td>';
					str += '<td>' + res[ z ].subject_name + '</td>';
					str += '<td>' + res[ z ].full_name + '</td>';
					str += '</tr>';
					no++;
				}
				str += '</tbody>';
...
<section class="content">
	<div class="callout callout-info">
   	...
   	<h4>Petunjuk Singkat</h4>
		<p>Menu ini digunakan untuk mengatur daftar <?=$this->session->userdata('_subject')?> pada tiap semester. Pastikan sebelum proses pembelajaran dimulai sudah mengatur daftar <?=$this->session->userdata('_subject')?> yang akan diajarkan dengan cara sebagai berikut :</p>
		<ol>
			<li>Pilih <?=$this->session->userdata('_subject')?> dengan cara menceklis daftar <?=$this->session->userdata('_subject')?> pada tabel disebelah kiri.</li>
			<li>Pilih <b><?=$this->session->userdata('_academic_year')?>, Semester</b>, dan <b>Kelas</b> pada tabel di sebelah kanan sebagai target penyimpanan <?=$this->session->userdata('_subject')?> yang terpilih / terceklis.</li>
			<li>Klik tombol <b>"SIMPAN KE DAFTAR <?=($this->session->userdata('school_level') >= 5 ? 'MATA KULIAH' : 'MATA PELAJARAN');?>"</b>.</li>
		</ol>
		<p>Jika ingin menduplikasi <?=$this->session->userdata('_subject')?> pada <?=$this->session->userdata('_academic_year')?>, semester, dan kelas sebelumnya, pada bagian <b>"Copy Data"</b> silahkan pilih <b>Ya</b>, kemudian pilih/ceklis <?=$this->session->userdata('_subject')?> dari <?=$this->session->userdata('_academic_year')?>, Semester, dan Kelas yang akan diduplikasi.</p>
	</div>
	<div class="row">
		<div class="col-md-5 col-sm-12 col-xs-12">
			<div class="box box-info">
				<div class="box-header with-border">
					<h3 class="box-title"><i class="fa fa-sign-out"></i> PILIH <?=strtoupper($this->session->userdata('_subject'))?></h3>
					<div class="box-tools">
						<div class="btn-group">
							<button class="btn btn-sm btn-success" onclick="Save()">SIMPAN KE DAFTAR <?=strtoupper($this->session->userdata('_subject'))?> <i class="fa fa-arrow-right"></i></button>
						</div>
					</div>
				</div>
				<div class="box-body">
					<form class="form-horizontal">
						<div class="box-body">
							...
							<div class="form-group from_academic_year_id" style="display: none;">
								<label for="from_academic_year_id" class="col-sm-5 control-label"><?=$this->session->userdata('_academic_year')?></label>
								<div class="col-sm-7">
		...
		<div class="col-md-7 col-sm-12 col-xs-12">
			<div class="box box-info">
				<div class="box-header with-border">
					<h3 class="box-title"><i class="fa fa-sign-out"></i> PENGATURAN <?=strtoupper($this->session->userdata('_subject'))?></h3>
					<div class="box-tools">
						<div class="btn-group">
				...
				<div class="box-body">
					<form class="form-horizontal">
						<div class="box-body">
							<div class="form-group">
								<label for="to_academic_year_id" class="col-sm-4 control-label"><?=$this->session->userdata('_academic_year')?></label>
								<div class="col-sm-8">
