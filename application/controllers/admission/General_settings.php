...
class General_settings extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'PENGATURAN UMUM PENERIMAAN '.strtoupper($this->session->userdata('_student')).' BARU ' . $this->session->userdata('admission_year');
		$this->vars['admission'] = $this->vars['admission_settings'] = $this->vars['admission_general_settings'] = true;
		$this->vars['content'] = 'admission/general_settings';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_settings->get_where($keyword, $limit, $offset, 'admission');
			$total_rows = $this->m_settings->total_rows($keyword, 'admission');
