...
class Post_categories extends Admin_Controller {
	...
	public function index() {
		...
		$this->vars['content'] = 'post_categories/read';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_post_categories->get_where($keyword, $limit, $offset);
			$total_rows = $this->m_post_categories->total_rows($keyword);
		...
		if ($this->input->is_ajax_request()) {
			...
			if ($this->validation()) {
				...
				if ($id !== 0 && ctype_digit((string) $id)) {
					...
					$response['insert_id'] = $this->m_post_categories->save($fill_data); 
					$response['type'] = $response['insert_id'] > 0 ? 'success' : 'error';
	...
	private function validation() {
		...
		$val->set_rules('category_name', 'Kategori', 'trim|required');
