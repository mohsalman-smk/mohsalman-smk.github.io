...
class Event extends Blog_Controller {
	...
	public function index() {
		...
		$this->vars['blog'] = $this->vars['event'] = $this->vars['all_event'] = true;
		$this->vars['content'] = 'event/read';
	...
	public function create() {
		...
		if ($id !== 0 && ctype_digit((string) $id)) {
			...
			$post_image = 'media_library/event/medium/'.$this->vars['query']->post_image;
		...
		$this->vars['title'] = $id && ctype_digit((string) $id) ? 'Edit Pengumuman' : 'Tambah Pengumuman';
		$this->vars['blog'] = $this->vars['event'] = $this->vars['event_create'] = true;
		$this->vars['action'] = site_url('blog/event/save/'.$id);
		$this->vars['content'] = 'event/create';
		$this->load->view('backend/index', $this->vars);
	}
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_event->get_where($keyword, $limit, $offset);
			$total_rows = $this->m_event->total_rows($keyword);
	...
	private function validation() {
		...
		$val->set_rules('waktu', 'Waktu', 'trim|required');
	...
	protected function upload_image($id) {
		...
		if (!$this->upload->do_upload('post_image')) {
			...
			if ($id > 0) {
				$query = $this->model->RowObject($this->table, $this->pk, $id);
				// chmood old file
				@chmod(FCPATH.'media_library/event/thumbnail/'.$query->post_image, 0777);
				@chmod(FCPATH.'media_library/event/medium/'.$query->post_image, 0777);
				@chmod(FCPATH.'media_library/event/large/'.$query->post_image, 0777);
				// unlink old file
				@unlink(FCPATH.'media_library/event/thumbnail/'.$query->post_image);
				@unlink(FCPATH.'media_library/event/medium/'.$query->post_image);
				@unlink(FCPATH.'media_library/event/large/'.$query->post_image);
			}
		}
		return $response;
	}
	...
	/**
	 ...
	 private function resize_image($source, $file_name) {
		...
		$thumb['new_image'] = './media_library/event/thumbnail/'. $file_name;
		...
		$medium['new_image'] = './media_library/event/medium/'. $file_name;
		...
		$large['new_image'] = './media_library/event/large/'. $file_name;
