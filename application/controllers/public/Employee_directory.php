...
class Employee_directory extends Public_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('public/m_employees');
		$this->total_rows = $this->m_employees->total_rows();
	...
	public function index() {
		$this->vars['page_title'] = 'DIREKTORI ' . ($this->session->userdata('school_level') >= 5 ? 'DOSEN DAN STAFF' : 'GURU DAN TENAGA KEPENDIDIKAN');
		$this->vars['total_pages'] = $this->total_pages;
		$this->vars['query'] = $this->m_employees->get_employees();
	...
	public function more_employees() {
		if ($this->input->is_ajax_request()) {
			...
			$response = [];
			$query = $this->m_employees->get_employees($offset);
			$rows = [];
