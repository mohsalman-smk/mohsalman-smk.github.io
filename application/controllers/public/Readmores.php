...
class Readmores extends Public_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model(['public/m_event', 'public/m_post_comments', 'm_settings']);
	...
	public function index() {
		...
		if ($id !== 0 && ctype_digit((string) $id)) {
			$this->vars['query'] = $this->model->RowObject('event', 'id', $id);
			...
			$this->m_event->increase_viewer($id);
			...
			$this->vars['content'] = 'themes/'.theme_folder().'/single-event';
