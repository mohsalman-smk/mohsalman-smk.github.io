	...
	public function save() {
		if ($this->input->is_ajax_request()) {
			...
			foreach($rows as $row) {
				...
				$fill_data['student_status_id'] = $this->m_options->get_options_id('student_status', 'lulus');
				$key_exist = $this->model->isValExist('identity_number', trim($exp[0]), 'students');
				$email_exist = $this->model->isValExist('identity_number', trim($exp[6]), 'students');
