...
class By_class_groups extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'GRAFIK BERDASARKAN KELAS ' . strtoupper($this->session->userdata('_student'));
		...
		$this->vars['content'] = 'students/by_class_groups';
	...
	public function generate_chart() {
		if ($this->input->is_ajax_request()) {
			...
			$response = [];
			$response['title'] = 'GRAFIK ' . strtoupper($this->session->userdata('_student')) . ' BERDASARKAN KELAS ' . strtoupper($this->session->userdata('_academic_year'));
			$response['labels'] = [];
			$response['data'] = [];
