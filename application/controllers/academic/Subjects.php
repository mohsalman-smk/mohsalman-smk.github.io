...
class Subjects extends Admin_Controller {
	...
	public function __construct() {
		...
		$this->pk = M_subjects::$pk;
		$this->table = M_subjects::$table;
	}
	...
	public function index() {
		$this->vars['title'] = 'MATA '. ($this->session->userdata('school_level') >= 5 ? 'KULIAH' : 'PELAJARAN');
		$this->vars['academic'] = $this->vars['academic_references'] = $this->vars['subjects'] = true;
		$this->vars['content'] = 'subjects/read';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_subjects->get_where($keyword, $limit, $offset);
			$total_rows = $this->m_subjects->total_rows($keyword);
	...
	private function validation() {
		...
		$val->set_rules('subject_name', 'Mata ' . ($this->session->userdata('school_level') >= 5 ? 'Kuliah' : 'Pelajaran'), 'trim|required');
	...
	public function get_subjects() {
		if ($this->input->is_ajax_request()) {
			$response = [];
			$query = $this->m_subjects->get_subjects();
			$this->output
				->set_content_type('application/json', 'utf-8')
				->set_output(json_encode($query, JSON_PRETTY_PRINT))
				->_display();
