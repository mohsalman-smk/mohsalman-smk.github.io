...
class Employment_types extends Admin_Controller {
	...
	public function index() {
		$this->vars['title'] = 'JENIS '.($this->session->userdata('school_level') >= 5 ? 'STAFF DAN DOSEN' : 'GURU DAN TENAGA KEPENDIDIKAN');
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_employment_types->get_where($keyword, $limit, $offset);
	...
	private function validation() {
		...
		$val->set_rules('option_name', 'Jenis ' . ($this->session->userdata('school_level') >= 5 ? 'Staff dan Dosen' : 'Guru dan Tenaga Kependidikan'), 'trim|required');
