...
class Special_needs extends Admin_Controller {
	...
	public function __construct() {
		parent::__construct();
		$this->load->model('m_special_needs');
		$this->pk = M_special_needs::$pk;
		$this->table = M_special_needs::$table;
	}
	...
	public function index() {
		...
		$this->vars['content'] = 'special_needs/read';
	...
	public function pagination() {
		if ($this->input->is_ajax_request()) {
			...
			$query = $this->m_special_needs->get_where($keyword, $limit, $offset);
			$total_rows = $this->m_special_needs->total_rows($keyword);
