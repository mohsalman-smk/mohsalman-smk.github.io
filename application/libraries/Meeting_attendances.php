...
class Meeting_attendances extends TCPDF {
	...
	public function create_pdf(array $params = []) {		
		...
		// Set Properties
		$this->SetTitle('LAPORAN DATA KEHADIRAN ' . strtoupper($this->CI->session->userdata('_student')));
		...
		// Header
		$content = str_replace('[TITLE]', 'LAPORAN DATA KEHADIRAN ' . strtoupper($this->CI->session->userdata('_student')), $content);
		...
		$content = str_replace('[STUDENT_ID]', $this->CI->session->userdata('_identity_number'), $content);
		$content = str_replace('[ACADEMIC_YEAR]', $this->CI->session->userdata('_academic_year'), $content);
