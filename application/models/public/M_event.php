...
class M_event extends CI_Model {
	...
	public function feed() {
		return $this->db
			->select('id, post_title, post_content, waktu, tempat, tanggal, bulan, tahun, post_slug, LEFT(created_at, 10) AS created_at')
	...
	public function get_related_event($categories = '', $id) {
		...
		$this->db->select('id, post_title, post_content, waktu, tempat, tanggal, bulan, tahun, LEFT(created_at, 10) AS created_at, post_image, post_slug, post_counter');
	 ...
	 * @return Resource
	 */
	public function total_rows($category_slug = '') {
		$id = 0;
		$query = $this->db
			->select('id')
			->where('category_slug', $category_slug)
	...
	public function more_archive_event($offset = 0, $year, $month) {
		$this->db->select('id, post_title, post_content, waktu, tempat, tanggal, bulan, tahun, LEFT(created_at, 10) AS created_at, post_image, post_slug');
		$this->db->where('post_type', 'post');
		$this->db->where('is_deleted', 'false');
		$this->db->where('post_status', 'publish');
