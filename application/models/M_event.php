...
class M_event extends CI_Model {
	...
	public function total_rows($keyword = '') {
		...
		return $this->db->count_all_results('event x1');
	}

	/**
	 * Get event for RSS Feed
	 * @return Resource
	 */
	public function feed() {
		return $this->db
			->select('id, post_title, waktu, tempat, tanggal, bulan, tahun, post_content, post_slug, LEFT(created_at, 10) AS created_at')
	 ...
	 * @return Resource
	 */
	public function get_recent_event($limit = 6) {
		$this->db->select('
			x1.id
		  , x1.post_title
		  , x1.waktu
	 ...
	 * @return Resource
	 */
	public function get_popular_event($limit = 6) {
		$this->db->select('
			x1.id
		  , x1.post_title
		  , x1.waktu
	...
	/**
	 * Get recent added event / for dashboard
	 * @access  Public
	 * @return Resource
	 */
	public function get_recent_added_event() {
	 ...
	 * @return Resource
	 */
	public function get_random_event($limit = 5) {
		$this->db->select('
			x1.id
		  , x1.post_title
		  , x1.waktu
	 ...
	 * @return 	Query
	 */
	public function get_related_event($categories = '', $id) {
		$categories = explode(',', $categories);
		$this->db->select('id, post_title, waktu, tempat, tanggal, bulan, tahun, post_content, LEFT(created_at, 10) AS created_at, post_image, post_slug, post_counter');
	 ...
	 * @return 	Resource
	 */
	public function get_archive_event($year, $month) {
		$this->db->select('
			x1.id
			, x1.post_title
			, x1.waktu
	...
	public function get_post_category($id, $limit= 0) {
		$this->db->select('x1.id, x1.post_title, x2.user_full_name AS author, x1.post_content, x1.waktu, x1.tempat, x1.tanggal, x1.bulan, x1.tahun, LEFT(x1.created_at, 10) AS created_at, x1.post_image, x1.post_slug, x1.post_counter');
		$this->db->join('users x2', 'x1.post_author = x2.id', 'LEFT');
		$this->db->where('x1.post_type', 'post');
		$this->db->where('x1.is_deleted', 'false');
		$this->db->where('x1.post_status', 'publish');
	...
	public function search($keyword) {
		$this->db->select('x1.id, x1.post_title, x2.user_full_name AS author, x1.post_content, x1.waktu, x1.tempat, x1.tanggal, x1.bulan, x1.tahun, LEFT(x1.created_at, 10) AS created_at, x1.post_slug, x1.post_counter');
		$this->db->join('users x2', 'x1.post_author = x2.id', 'LEFT');
		$this->db->where('x1.post_status', 'publish');
		if (! $this->auth->is_logged_in()) {
			$this->db->where('x1.post_visibility', 'public');
	 ...
	 * @return Resource
	 */
	public function more_event($category_slug = '', $offset = 0) {
		$id = 0;
		$query = $this->db
			->select('id')
		...
		$this->db->select('x1.id, x1.post_title, x1.post_content, x1.waktu, x1.tampat, x1.tanggal, x1.bulan, x1.tahun, LEFT(x1.created_at, 10) AS created_at, x1.post_image, x1.post_slug, x1.post_counter');
		$this->db->where('x1.post_type', 'post');
		$this->db->where('x1.is_deleted', 'false');
		$this->db->where('x1.post_status', 'publish');
		if (! $this->auth->is_logged_in()) {
	 ...
	 * @return 	Resource
	 */
	public function more_archive_event($offset = 0, $year, $month) {
		$this->db->select('id, post_title, post_content, waktu, tempat, tanggal, x1.bulan, tahun, LEFT(created_at, 10) AS created_at, post_image, post_slug');
		$this->db->where('post_type', 'post');
		$this->db->where('is_deleted', 'false');
		$this->db->where('post_status', 'publish');
	 ...
	 * @return Resource
	 */
	public function more_event_by_tag($tag = '', $offset = 0) {
		$this->db->select('x1.id, x1.post_title, x1.post_content, x1.waktu, x1.tempat, x1.tanggal, x1.bulan, x1.tahun, LEFT(x1.created_at, 10) AS created_at, x1.post_image, x1.post_slug, x1.post_counter');
		$this->db->where('x1.post_type', 'post');
		$this->db->where('x1.is_deleted', 'false');
		$this->db->where('x1.post_status', 'publish');
		if (! $this->auth->is_logged_in()) {
